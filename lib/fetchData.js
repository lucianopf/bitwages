...
function logOutput(data, sourceValues) {
  return {
    ...
    USDBRLACTIVEACTION: data.sort((a, b) => a.USDBRL < b.USDBRL).map(el => {
    ...
    USDBRLPASSIVEACTION: data.sort((a, b) => a.USDBRL < b.USDBRL).map(el => {
      return Object.assign(
        {},
        {
          id: el.id,
          value: parseFloat(el.USDBRL - el.USDBRL * el.passiveAction).toFixed(
            2
          ),
        }
      )
...
function fetchUSDBRL() {
  return fetch('https://api.bitwage.com/v1/tickers').then(parseRequest)
}

function gatherAvgUSDBRL(response) {
  return parseFloat(_get(response, 'USDBRL')) * 0.96
}

function calculateUSDBRL(data, sourceValues) {
  return fetchUSDBRL().then(response => {
    const USD = gatherAvgUSDBRL(response)
    sourceValues.USD = USD.toFixed(2).toString()
    return data.map(exchange => {
      exchange.USDBRL = parseFloat(
        (exchange.percentageOverBitwage / 100) * USD
      ).toFixed(2)
      return exchange
    })
  })
...
function calculatePercentages(data) {
  const bitwagePrice = _get(
    data.filter(el => el.id === 'Bitwage'),
    '0.priceAfterTaxes'
  )
  return data.map(el => {
...
function calculateRawDolar(response, sourceValues) {
  const bitwageBTC = (sourceValues.BITWAGE_XBTUSD = _get(
    JSON.parse(response[0]),
    'XBTUSD'
  ))
  const WallTime = parseFloat(response[1])
  return fetchUSDBRL().then(responseDollar => {
...
function getLastRatios(data) {
  return data.map(el =>
    Object.assign({ id: _get(el, 'id'), price: _get(el, 'last') })
  )
}
...
function getExchanges(data) {
  ...
  _get(data, 'ticker_1h.exchanges') ||
    handleError('nodata').forEach((el, key) => {
      exchanges[key] = el
      return el
    })
  return exchanges
