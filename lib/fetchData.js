...
function applyTaxes (data, exchangeTaxes) {
  return data.map(el => {
    ...
    if (el.id === 'Bitwage') {
      console.log(el.price, elTaxes.transferBTC, el.price * elTaxes.transferBTC)
...
function calculateRawDolar (response, sourceValues) {
  ...
  return fetchUSDBRL()
    .then(responseDollar => {
      ...
      console.log('Bitwage', bitwageBTC)
      console.log('USD', USD)
      return data.concat([
        {
          id: 'Bitwage',
...
function getExchanges (data) {
  ...
  _get(data, 'ticker_1h.exchanges') || handleError('nodata').forEach((el, key) => exchanges[key] = el)
  return exchanges
}
